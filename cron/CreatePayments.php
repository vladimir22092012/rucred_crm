    ...
    private function run()
    {
        ...
        if (!empty($contracts)) {
            ...
            foreach ($contracts as $contract) {
                ...
                $template = $this->sms->get_template(9);
                $replace_from =
                    [
                        '$date',
                        '$pay_sum'
                ...
                $replace_to =
                    [
                        date('d.m.Y', strtotime($contract->return_date)),
                        number_format($next_payment['pay_sum'], 2, ',', ' ')
                    ];

                $message = str_replace($replace_from, $replace_to, $template->template);
            ...
            $this->Logs->add(['text' => json_encode($payment)]);

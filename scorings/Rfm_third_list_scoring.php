...
class Rfm_third_list_scoring extends Core
{
    private $user_id;
    private $order_id;
    private $audit_id;
    ...
    {
        $update = array();
        $tempScore = RfmscoringORM::query()->where('type', '=', 'third_list')->first();

        $scoring_type = $this->scorings->get_type('rfm_third_list');
            ...
            if ($order = $this->orders->get_order((int)$scoring->order_id))
            {
                ...
                else
                {
                    ...
                    if ($score)
                    {
                        ...
                        $update['string_result'] = '<p>Клиент найден в списке: Список ООН</p>'.
                        ...
                        $update['string_result'] = '<p>Клиент не найден в списке: Список ООН</p>'.'<p>Версия файла: ' . $tempScore->created.'</p>';

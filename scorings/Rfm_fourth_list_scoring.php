...
class Rfm_fourth_list_scoring extends Core
{
    private $user_id;
    private $order_id;
    private $audit_id;
    ...
    {
        $update = array();
        $tempScore = RfmscoringORM::query()->where('type', '=', 'fourth_list')->first();
        $scoring_type = $this->scorings->get_type('rfm_fourth_list');
            ...
            if ($order = $this->orders->get_order((int)$scoring->order_id))
            {
                ...
                else
                {
                    ...
                    if ($score)
                    {
                        ...
                        $update['string_result'] = '<p>Клиент найден в списке: Решение судов</p>'.
                        ...
                        $update['string_result'] = '<p>Клиент не найден в списке: Решение судов</p>'.'<p>Версия файла: ' . $tempScore->created.'</p>';

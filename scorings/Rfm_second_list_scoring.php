...
class Rfm_second_list_scoring extends Core
{
    private $user_id;
    private $order_id;
    private $audit_id;
    ...
    {
        $update = array();
        $tempScore = RfmscoringORM::query()->where('type', '=', 'second_list')->first();

        $scoring_type = $this->scorings->get_type('rfm_second_list');
            ...
            if ($order = $this->orders->get_order((int)$scoring->order_id))
            {
                ...
                else
                {
                    ...
                    if ($score)
                    {
                        ...
                        $update['string_result'] = '<p>Клиент найден в списке: Перечень о замораживании средств</p>'.
                        ...
                        $update['string_result'] = '<p>Клиент не найден в списке: Перечень о замораживании средств</p>'.'<p>Версия файла: ' . $tempScore->created.'</p>';

...
class TestController extends Controller
{
    ...
    private function parseExcell()
    {
        ...
        for ($row = $first_row; $row <= $last_row; $row++) {
            ...
            $birth = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($active_sheet->getCell('M' . $row)->getValue());
            ...
            $passport_date = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($active_sheet->getCell('R' . $row)->getValue());
            $snils = $active_sheet->getCell('S' . $row)->getValue();
            $inn = $active_sheet->getCell('T' . $row)->getValue();

            $lastname_spouse = $active_sheet->getCell('W' . $row)->getValue();
            $firstname_spouse = $active_sheet->getCell('X' . $row)->getValue();
            $patronymic_spouse = $active_sheet->getCell('Y' . $row)->getValue();
            $phone_spouse = $active_sheet->getCell('Z' . $row)->getValue();
            ...
            $regStreet = explode(' ', $active_sheet->getCell('Y' . $row)->getValue());

            $regaddress['street'] = $regStreet[1];
            $regaddress['street_type'] = $regStreet[0];
            ...
            UsersORM::where('id', $user->id)->update(['regaddress_id' => $regAddrId, 'faktaddress_id' => $faktAddrId]);

            $number_acc = $active_sheet->getCell('AA' . $row)->getValue();
            $bik = $active_sheet->getCell('AB' . $row)->getValue();
            $name = $active_sheet->getCell('AC' . $row)->getValue();

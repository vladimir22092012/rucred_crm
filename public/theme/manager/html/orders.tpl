    ...
    <div class="container-fluid">
        ...
        <div class="row page-titles">
            <div class="col-md-6 col-8 align-self-center">
                <h3 class="text-themecolor mb-0 mt-0"><i class="mdi mdi-animation"></i> Заявки {if $offline}оффлайн{/if}</h3>
                <ol class="breadcrumb">
            ...
            <div class="col-md-6 col-4 align-self-center">
                <div class="row">
                    ...
                    <div class="col-6 dropdown text-right hidden-sm-down ">
                        ...
                        <div class="js-period-filter">
                            ...
                            <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                ...
                                {elseif $issuance_period == 'week'}На этой неделе
                                {elseif $issuance_period == 'month'}В этом месяце
                                {elseif $issuance_period == 'year'}В этом году
                    ...
                    <div class="col-6 dropdown text-right hidden-sm-down">
                        ...
                        <div class="js-daterange-filter input-group mt-3" {if $period!='optional'}style="display:none"{/if}>
                            <input type="text" name="daterange" class="form-control daterange js-daterange-input" value="{if $from && $to}{$from}-{$to}{/if}">
                            <div class="input-group-append">
                                <span class="input-group-text">
                                    <span class="ti-calendar"></span>
                    ...
                    <div class="card-body">
                        ...
                        <div class="clearfix">
                            <div class="js-filter-status mb-2 float-left">
                                ...
                                <a href="{if $filter_status==2}{url status=null page=null}{else}{url status=2 page=null}{/if}" class="btn btn-xs {if $filter_status==2}btn-success{else}btn-outline-success{/if}">Одобрена</a>
                                ...
                                <a href="{if $filter_status==4}{url status=null page=null}{else}{url status=4 page=null}{/if}" class="btn btn-xs {if $filter_status==4}btn-inverse{else}btn-outline-inverse{/if}">Подписан</a>
                                <a href="{if $filter_status==5}{url status=null page=null}{else}{url status=5 page=null}{/if}" class="btn btn-xs {if $filter_status==5}btn-primary{else}btn-outline-primary{/if}">Выдан</a>
                                <a href="{if $filter_status==6}{url status=null page=null}{else}{url status=6 page=null}{/if}" class="btn btn-xs {if $filter_status==6}btn-danger{else}btn-outline-danger{/if}">Не удалось выдать</a>
                                <a href="{if $filter_status==7}{url status=null page=null}{else}{url status=7 page=null}{/if}" class="btn btn-xs {if $filter_status==7}btn-inverse{else}btn-outline-inverse{/if}">Погашен</a>
                                <a href="{if $filter_status==8}{url status=null page=null}{else}{url status=8 page=null}{/if}" class="btn btn-xs {if $filter_status==8}btn-danger{else}btn-outline-danger{/if}">Отказ клиента</a>
                                {*}
                                {if $filter_status}
                            ...
                            <div class="float-right js-filter-client">
                                <a href="{if $filter_client=='new'}{url client=null page=null}{else}{url client='new' page=null}{/if}" class="btn btn-xs {if $filter_client=='new'}btn-info{else}btn-outline-info{/if}">Новая</a>
                                <a href="{if $filter_client=='repeat'}{url client=null page=null}{else}{url client='repeat' page=null}{/if}" class="btn btn-xs {if $filter_client=='repeat'}btn-warning{else}btn-outline-warning{/if}">Повтор</a>
                                <a href="{if $filter_client=='pk'}{url client=null page=null}{else}{url client='pk' page=null}{/if}" class="btn btn-xs {if $filter_client=='pk'}btn-success{else}btn-outline-success{/if}">ПК</a>
                                {if $filter_client}
                            ...
                            <div class="jsgrid-grid-header jsgrid-header-scrollbar">
                                <table class="jsgrid-table table table-striped table-hover">
                                    <tr class="jsgrid-header-row">
                                        ...
                                        <th style="width: 150px;" class="jsgrid-header-cell jsgrid-header-sortable {if $sort == 'employer_asc'}jsgrid-header-sort jsgrid-header-sort-asc{elseif $sort == 'employer_desc'}jsgrid-header-sort jsgrid-header-sort-desc{/if}">
                                            {if $sort == 'employer_asc'}<a href="{url page=null sort='employer_desc'}">Работодатель</a>
                                            {else}<a href="{url page=null sort='employer_asc'}">Работодатель</a>{/if}
                                        </th>
                                        ...
                                        {if $manager->role == 'quality_control' || $manager->role == 'quality_control_plus'}
                                        <th style="width: 80px;" class="jsgrid-header-cell {if $sort == 'penalty_asc'}jsgrid-header-sort jsgrid-header-sort-asc{elseif $sort == 'penalty_desc'}jsgrid-header-sort jsgrid-header-sort-desc{/if}">
                                            {if $sort == 'penalty_asc'}<a href="{url page=null sort='penalty_desc'}">Дата решения</a>
                                    ...
                                    <tr class="jsgrid-filter-row" id="search_form">
                                        ...
                                        <td style="width: 150px;" class="jsgrid-cell">
                                            <input type="text" name="employer" value="{$search['employer']}" class="form-control input-sm">
                                        </td>
                                        <td style="width: 70px;" class="jsgrid-cell">

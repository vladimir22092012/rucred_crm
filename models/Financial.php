...
class Financial extends Core
{
    ...
    function DATEADD($datepart, $number, $date)
    {
        ...
        switch (strtolower($datepart)) {
            ...
            case 'year':
                $d = getdate($date);
                $d['year'] += $number;
                if (($d['mday'] == 29) && ($d['mon'] == 2) && (date('L', mktime(0, 0, 0, 1, 1, $d['year'])) == 0)) {
                    $d['mday'] = 28;
                }
    ...
    */
    ...
    {
        $t = time();
        if (!isset($refPeriodStart)) {
            $refPeriodStart = $startdate;
        }
        if (!isset($refPeriodEnd)) {
            $refPeriodEnd = $enddate;
        }
        ...
        */
        ...
        if ($this->DATEDIFF('day', $startdate, $enddate) == 0) {
            return 0.0;
        }
        ...
        $period = $months / 12.0;
        if ($months == 0) {
            die("number of months does not divide 12 exactly");
        }
        if ($enddate <= $refPeriodEnd) {
            ...
            if ($startdate >= $refPeriodStart) {
                ...
                if ($enddate > $refPeriodStart) {
                    return $this->ActualActualyearFraction(
                        $startdate,
                        $refPeriodStart,
                        $previousRef,
                        $refPeriodStart
                    ) +
                            $this->ActualActualyearFraction(
                                $refPeriodStart,
                                $enddate,
                                $refPeriodStart,
                                $refPeriodEnd
    ...
    */
    function _calculate_fvifa($rate, $nper)
    {
        ...
        if ($rate == 0) {
            return $nper;
        } else { // FIXME: this sucks for very small rates
    ...
    */
     ...
     *
     */
    function ELD($settlement, $maturity, $rate, $pr, $redemption, $frequency, $basis = FINANCIAL_BASIS_MSRB_30_360)
    {
        if (!$this->_is_valid_basis($basis)) {
            return null;
        }
        if (!$this->_is_valid_frequency($frequency)) {
            return null;
        }
        if ($rate < 0) {
            return null;

...
class YaDisk extends Core
{
    ...
    private function upload($upload_scans, $order, $resource, $file_name, $document)
    {
        ...
        if ($upload_scans == 1)
            $resource->upload($this->config->__get('root_url') . '/files/users/' . $order->user_id . '/' . $document->name, true);
        else {
            ...
            $resource->upload($this->config->__get('root_url') . '/files/users/' . $file_name . '.pdf', true);
            unlink($this->config->__get('root_url'). '/files/users/' . $file_name . '.pdf');
        }
    }
